...
c = session.query(Project).filter_by(project_item_id = project_no).all();
print c
# Get a cursor object
...
c = session.query(Project).filter_by(projectname_id = "frontend").all()
for i in c:
...
c = session.query(Project).filter_by(project_item_id = 1).all();
for i in c:
	print "project item id: ", i.project_item_id
	print "project url: ", i.project_url
	print "project category id: ", i.projectcategory_id
